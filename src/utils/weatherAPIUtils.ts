...
const fetchWeatherData = async (url: string) => {
  try {
    ...
    console.log(`Error fetching weather data: ${error.message ? error.message : error}`);
  }
}
...
export const getData = async (
  ...
  [key: string]: string,
    ...
    const currentData = await getFetchUrl(baseUrl, ...args).then(fetchWeatherData);
    const forecastData = await getFetchUrl(forecastUrl, ...args).then(fetchWeatherData);
